"""Shared testcontainers fixtures for property-based Neo4j tests.

Fixtures are session-scoped so hypothesis can run many examples against the
same container. Each example must use a unique ``source_id`` and clean up its
own data to keep examples isolated.
"""
...
def neo4j_container(neo4j_image: str) -> Any:
    """Spin up one throwaway Neo4j container for the whole property suite."""
